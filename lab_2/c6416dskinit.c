...
void c6416_dsk_init() // dsp-peripheral initialization
{
	...
	MCBSP_start(DSK6416_AIC23_DATAHANDLE, MCBSP_XMIT_START | MCBSP_RCV_START | MCBSP_SRGR_START | MCBSP_SRGR_FRAMESYNC, 220); // start data channel again
}
...
void output_sample(Uint32 out_data) // for out to Left and Right channels
{
	...
	if (poll)
		while (!MCBSP_xrdy(DSK6416_AIC23_DATAHANDLE))
...
Uint32 input_sample() // for 32-bit input
{
	...
	if (poll)
		while (!MCBSP_rrdy(DSK6416_AIC23_DATAHANDLE))
			;											  // if ready to receive
